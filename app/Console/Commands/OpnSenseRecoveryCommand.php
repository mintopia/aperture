...
class OpnSenseRecoveryCommand extends Command
{
    ...
    public function handle(OpnSense $opnSense): int
    {
        ...
        if ($uptime > 3600) {
            ...
            $this->log(sprintf('Uptime is %d, this is more than an hour, assuming no reboot', $uptime));
        ...
        if ($uptime > $lastUptime) {
            ...
            $this->log(sprintf('Uptime is %d, this is more than the last uptime', $uptime));

            return self::SUCCESS;
        }

        $this->log(sprintf('Uptime is %d, this is less than the last uptime, restoring IPs', $uptime));
        $ips = IpAddress::all();
        foreach ($ips as $ip) {
            $this->processIp($ip);
        }
